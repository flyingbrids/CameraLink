...
u32 PcapDataTransfer(u32 *SourceDataPtr, u32 *DestinationDataPtr,
                     u32 SourceLength, u32 DestinationLength,
                     u32 SecureTransfer) {
  u32 Status;
  u32 IntrStsReg;
  ...
  if (SecureTransfer) {
	...
	if (Status != XST_SUCCESS) {
		xil_printf("Status of XDcfg_Transfer = %lu \r \n",Status);
		return XST_FAILURE;
	}
		...
		u32 SourceLength, u32 DestinationLength, u32 SecureTransfer)
{
	 ...
	 * PCAP single DMA transfer setup
	 */
	u32 * SourceDataPtr1 = (u32*)((u32)SourceDataPtr | PCAP_LAST_TRANSFER);
	DestinationDataPtr = (u32*)((u32)DestinationDataPtr | PCAP_LAST_TRANSFER);

    xil_printf("Wait for file");
    Status = TeraTermFile_Receive ((SourceLength << 2), SourceDataPtr);
	/*
	 * Transfer using Device Configuration
	 */
	Status = XDcfg_Transfer(DcfgInstPtr, (u8 *)SourceDataPtr1,
					SourceLength,
					(u8 *)DestinationDataPtr,
					DestinationLength, PcapTransferType);
	if (Status != XST_SUCCESS) {
	...
	if (Status != XST_SUCCESS) {
		xil_printf("PCAP_FPGA_DONE_FAIL\r\n");
		return XST_FAILURE;
	}

	xil_printf("FPGA Done ! \n\r");
...
int InitPcap(u32 *done)
{
    ...
    *done = XDcfg_IntrGetStatus(DcfgInstPtr) & XDCFG_IXR_PCFG_DONE_MASK;
...
u32 FabricInit(void)
{
	...
	xil_printf("Level Shifter Value = 0x%lx \r\n",
    ...
    */
	...
	xil_printf("Devcfg Status register = 0x%lx \r\n",StatusReg);
...
u32 ClearPcapStatus(void)
{
	...
	if (IntStatusReg & FSBL_XDCFG_IXR_ERROR_FLAGS_MASK) {
		xil_printf("FATAL errors in PCAP %lx\r\n",
				IntStatusReg);
		return XST_FAILURE;
	}
	...
	xil_printf("PCAP:StatusReg = 0x%.8lx\r\n", StatusReg);
...
int TeraTermFile_Receive ( u32 NoByteToRead, u32 *DestinationDataPtr)
{
      // UART Init 
    XUartPs Uart_Ps;
	...
	XUartPs_WriteReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_CR_OFFSET,
     ...
     u32 FileByteCount=0;
     Xil_DCacheFlushRange((UINTPTR)DestinationDataPtr, NoByteToRead);
     unsigned char *DDR_MEMB0  = (unsigned char* )DestinationDataPtr;
     unsigned char temp[4];
     unsigned int temp_cnt = 0;
     while (FileByteCount < NoByteToRead-4)
  	{
    	while (!XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR));         
        #ifdef SWAP                   
                temp[temp_cnt]  = XUartPs_ReadReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_FIFO_OFFSET);
                temp_cnt++;   
                if (temp_cnt == 4) {
                    temp_cnt = 0;
                    DDR_MEMB0[FileByteCount] = temp[3];
                    DDR_MEMB0[FileByteCount+1] = temp[2];
                    DDR_MEMB0[FileByteCount+2] = temp[1];
                    DDR_MEMB0[FileByteCount+3] = temp[0];
    			    FileByteCount = FileByteCount + 4;
                } 
        #else 
                DDR_MEMB0[FileByteCount++] = XUartPs_ReadReg(XPAR_XUARTPS_0_BASEADDR, XUARTPS_FIFO_OFFSET);
        #endif       
  	}

    DDR_MEMB0[FileByteCount]   = 0x00;
    DDR_MEMB0[FileByteCount+1] = 0x00;
    DDR_MEMB0[FileByteCount+2] = 0x00;
    DDR_MEMB0[FileByteCount+3] = 0x20; 
