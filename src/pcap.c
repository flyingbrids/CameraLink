     ...
     while (FileByteCount < NoByteToRead-4)
  	{
        ...
        #ifdef SWAP                   
                temp[temp_cnt]  = XUartPs_ReadReg(XPAR_XUARTPS_1_BASEADDR, XUARTPS_FIFO_OFFSET);
                temp_cnt++;   
                if (temp_cnt == 4) {
                    temp_cnt = 0;
        ...
        #else 
                DDR_MEMB0[FileByteCount++] = XUartPs_ReadReg(XPAR_XUARTPS_1_BASEADDR, XUARTPS_FIFO_OFFSET);
        #endif       
  	}
