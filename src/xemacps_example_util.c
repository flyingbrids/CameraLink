...
char RemoteMAC[] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
//char RemoteMAC[] = { 0x80, 0x3F, 0x5D, 0xF2, 0xFE, 0x04 };
u32 Platform;
...
*
*****************************************************************************/
void EmacPsUtilFrameSetPayloadData(EthernetFrame *FramePtr, u32 PayloadSize, u32 Offset)
{
	u32 BytesLeft = PayloadSize;	
    unsigned char *DDR_MEMB0 = PAYLOAD_BUFFER + Offset;
	u8 *Frame; 
    ...
    IPV4HeaderInit(BytesLeft, Frame);
    BytesLeft -= IPV4HEADERSIZE;

    Frame += IPV4HEADERSIZE;
    UDPHeaderInit(BytesLeft, Frame);
    BytesLeft -= UDPHEADERSIZE;
...
//	u16 Counter;
    ...
    CheckPtr = &CheckPtr[14];

    while (BytesLeft > 0) {
         if (*CheckPtr++ != *ActualPtr++){
             return XST_FAILURE;
         }
         BytesLeft --;
    }
...
LONG EmacPsUtilMarvellPhyLoopback(XEmacPs *EmacPsInstancePtr,
				  u32 Speed, u32 PhyAddr, u8 loopbackEn)
{
	LONG Status;
	u16 PhyReg0  = 0;
	u16 PhyReg21  = 0;
...
LONG EmacPsUtilEnterLoopback(XEmacPs *EmacPsInstancePtr, u32 Speed, u8 loopbackEn)
{
	LONG Status;
	u16 PhyIdentity;
	u32 PhyAddr;
	...
	if (PhyIdentity == PHY_ID_MARVELL) {
		Status = EmacPsUtilMarvellPhyLoopback(EmacPsInstancePtr, Speed, PhyAddr, loopbackEn);
	}
